...
public class DbHelper extends SQLiteOpenHelper implements DbInterface {
	...
	@Override
	public ArrayList<RezultEnt> user_get_all_rezult() {	//зчитування всіх результатів користувача 
		ArrayList<RezultEnt> list = new ArrayList<RezultEnt>();
        String selectQuer = "SELECT * FROM "+TABLE_REZULT+";";
	...
	public ArrayList<RezultEnt> day_get_middle_rezult(ArrayList<String> date) {
		...
		for (int i=0;i<date.size();i++){
			...
			String selectQuer = "SELECT * FROM " + TABLE_REZULT+" WHERE " + REZULT_DATE + "='" + date.get(i)+"';";
	        Cursor cursor = db.rawQuery(selectQuer, null);    
	...
	public ArrayList<WordTranslEnt> get_all_words() {
		...
		db= this.getReadableDatabase();
		String selectQuer = "SELECT * FROM "  + TABLE_WORD_TRANSLATE + ";";
        Cursor cursor = db.rawQuery(selectQuer, null);    
        if (cursor.moveToFirst()){
            do{
